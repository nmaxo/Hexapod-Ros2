...
# Control table ADDRess for AX-18
...
class Ax18:
    """ Class for Dynamixel AX18A motors.

    """
    DXL_MIDDLE_POSITION_VALUE  = 510
    PROTOCOL_VERSION = 1.0
    BAUDRATE = 500000  # Dynamixel default baudrate
    ...
    def __repr__(self):
        return "Ax18('{}')".format(self.id)

    # functions to read/write to registers
    def set_register1(self, reg_num, reg_value):
        dxl_comm_result, dxl_error = Ax18.packetHandler.write1ByteTxRx(
            Ax18.portHandler, self.id, reg_num, reg_value)
        Ax18.check_error(dxl_comm_result, dxl_error)

    def get_register1(self, reg_num):
        reg_data, dxl_comm_result, dxl_error = Ax18.packetHandler.read1ByteTxRx(
            Ax18.portHandler, self.id, reg_num)
        Ax18.check_error(dxl_comm_result, dxl_error)
        return reg_data

    def set_register2(self, reg_num, reg_value):
        dxl_comm_result, dxl_error = Ax18.packetHandler.write2ByteTxRx(
            Ax18.portHandler, self.id, reg_num, reg_value)
        Ax18.check_error(dxl_comm_result, dxl_error)

    def get_register2(self, reg_num_low):
        reg_data, dxl_comm_result, dxl_error = Ax18.packetHandler.read2ByteTxRx(
            Ax18.portHandler, self.id, reg_num_low)
        Ax18.check_error(dxl_comm_result, dxl_error)
        return reg_data
    ...
    def connect(cls):
        ...
        cls.groupSyncWrite = GroupSyncWrite(cls.portHandler, cls.packetHandler, 
                                            ADDR_AX_GOAL_POSITION_L, 2)  # 2 байта на позицию
        
    @classmethod
    def sync_write_goal_position(cls, motor_ids, positions):
        """
        Устанавливает целевые позиции для нескольких моторов с помощью SYNC_WRITE.
        :param motor_ids: список ID моторов (например, [1, 2, 3, ..., 18])
        :param positions: список позиций (например, [500, 550, 600, ...])
        :return: True если успешно, False если ошибка
        """
        if len(motor_ids) != len(positions):
            raise ValueError("Количество ID и позиций должно совпадать")

        # Очистка предыдущих параметров
        cls.groupSyncWrite.clearParam()

        # Добавление параметров для каждого мотора
        for motor_id, pos in zip(motor_ids, positions):
            # Позиция AX-18 — 2 байта (0–1023), разбиваем на Low и High
            param_goal_position = [DXL_LOBYTE(pos), DXL_HIBYTE(pos)]
            success = cls.groupSyncWrite.addParam(motor_id, param_goal_position)
            if not success:
                print(f"[ID:{motor_id}] Failed to add sync write parameter")
                return False

        # Отправка пакета SYNC_WRITE
        dxl_comm_result = cls.groupSyncWrite.txPacket()
        if dxl_comm_result != COMM_SUCCESS:
            print(f"Sync Write failed: {cls.packetHandler.getTxRxResult(dxl_comm_result)}")
            return False

        if cls.DEBUG:
            print(f"Sync Write successful for motors: {motor_ids} with positions: {positions}")
    ...
    def check_error(comm_result, dxl_err):
        if comm_result != COMM_SUCCESS:
            print("%s" % Ax18.packetHandler.getTxRxResult(comm_result))
        elif dxl_err != 0:
            print("%s" % Ax18.packetHandler.getRxPacketError(dxl_err))
